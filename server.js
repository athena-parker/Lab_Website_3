...
app.get('/team_stats', function(req, res) {
	...
	var query3 = 'SELECT COUNT(visitor_score) FROM football_games WHERE visitor_score > home_score;';
	db.task('get-everything', task => {
	    return task.batch([
		task.any(query1),
		task.any(query2),
...
app.get('/player_info/post', function(req, res) {
	...
	var query2 = 'SELECT * FROM football_players WHERE id = ' + choice.toString() + ';';
	var query3 = 'SELECT COUNT(*) FROM football_games INNER JOIN football_players ON football_players.id = ANY (football_games.players) WHERE football_players.id = ' + choice.toString() + ';';
	db.task('get-everything', task => {
	    return task.batch([
		task.any(query1),
		task.any(query2),
